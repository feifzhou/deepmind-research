...
device=0
for nlayer in 8 6 4 3; do 
for buf in 1; do
    for N1 in 2; do
        for lr in 3e-4; do # 1e-2 3e-3 1e-3; do
            for nfeat in 180 ; do #150 180 80 128
for symm in cubic ; do #""
for act in raf1; do #swish
for threshold in 1e-3; do 
            DIR=meshgraphnets/experiment/grain-nfeat${nfeat}_nlayer${nlayer}_symm${symm}_act${act}_threshold${threshold}
            mkdir -p $DIR
 ...
 --mode=train \
 --num_training_steps=500000 --batch=8 --lr=$lr --lr_decay=70000 --cache --keep_ckpt=25 --rotate=$symm --mlp_activation=$act \
