...
for buf in 0 1; do
    for N1 in 2; do
        for freq in 1 20  999; do # 1e-2 3e-3 1e-3; do
 ...
 --amr_N=64 --amr_N1=$N1 --amr_buffer=$buf --amr_threshold=1e-3 --amr_eval_freq=$freq \
 --mode=eval \
 --num_training_steps=500000 --batch=6 --lr=1e-4 --lr_decay=100000 --cache \
 --noise=0.02 --rollout_split=test --rollout_path=$DIR/rollout.pkl --num_rollouts=10 &> tmp.txt
 t1=`grep 'Rollout trajectory 1' tmp.txt |sed 's/:/ /g'|awk '{print $2*3600 + $3*60 + $4}'`
 t2=`grep 'Rollout trajectory 9' tmp.txt |sed 's/:/ /g'|awk '{print $2*3600 + $3*60 + $4}'`
